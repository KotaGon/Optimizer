...
namespace OptimizeName 
{
  ...
  void Simplex::opt()
  {
    ...
    map<int, map<int, int> > varInd2ConstIndMap;

    for(int ic = 0; ic < Nconstraint; ++ic)
    {
      Function *g = &pB->at(ic);
      
      for(map<int, double>::iterator ite = g->CoeffMap.begin(); ite != g->CoeffMap.end(); ++ite)
        varInd2ConstIndMap[ite->first][ic] = ic;  
    }

    while(1)
    {
      clock_t time1 = clock();
      xpivod = pfobj->getXpivodCol_r2(*pB, varInd2ConstIndMap);
      if(xpivod < 0) break;
      ...
      bool varFlag = false;
      //pivodrow  = pfobj->getXpivodRow_r2(xpivod, *pB, varInd2ConstIndMap, varFlag);
      pivodrow  = pfobj->getXpivodRow_r2(xpivod, *pB, varInd2ConstIndMap);
      ...
      for(map<int, int>::iterator ite = varInd2ConstIndMap[xpivod].begin(); ite !=  varInd2ConstIndMap[xpivod].end(); ++ite)
      {
        if(ite->first == xpivodrow) continue;
	pB->at(ite->first).SwapVariable_r2(xpivod, ix, ite->first, pB->at(xpivodrow), varInd2ConstIndMap);
      }
  ...
  void Simplex::optRevised()
  {
    ...
    this->getFeasibleInitilize(basicVariableIdx, nonbasicVariableIdx);
    cout << "end get feasible sol" << endl;
  ...
  int Function::SwapVariable_r2(int xpivod, int ix, int ic, Function &Bi, map<int, map<int, int> > &varInd2ConstIndMap)
  {
    ...
    {
      int index = ite->first; 
      varInd2ConstIndMap[index][ic] = ic;
	...
	if(fabs(this->CoeffMap[index]) < eps)
	{
	  this->setCoeff(index, 0.0); 
	  this->eraseCoeff(index);

	  varInd2ConstIndMap[index].erase(ic);
	}
  ...
  int Function::getXpivodCol(vector<Function> &pB)
  {
    ...
    if(this->getobject() == maximization) maxval = -100.0;

    for(map<int, double>::iterator ite = this->CoeffMap.begin(); ite != this->CoeffMap.end(); ++ite)
    {
      double a2 = 1.0;

      if(this->getobject() == maximization && ite->second < 0)
	continue;
      if(this->getobject() == minimization && ite->second > 0)
	continue;

      if(steepest_edge)
      {
	for(int ib = 0; ib < pB.size(); ++ib)
	{
	  if(pB[ib].CoeffMap.find(ite->first) != pB[ib].CoeffMap.end())
	  {
	    a2 += sq(pB[ib].CoeffMap[ite->first]);
	  }
	}
	a2 = sqrt(a2);
      }

      if(this->getobject() == maximization)
      {
	double tmpval = (ite->second);
	tmpval = tmpval / a2;
	
	if(tmpval > 0 && maxval < tmpval)
	{
	  maxval = tmpval;
	  ix = ite->first;
	  xpivod = ix;
      ...
      else if(this->getobject() == minimization)
      {
        double tmpval = (ite->second);
	tmpval = tmpval / a2;
  ...
  int Function::getXpivodCol_r2(vector<Function> &pB, map<int, map<int, int> > &varInd2ConstMap)
  {
    ...
    if(this->getobject() == maximization) maxval = -1000000000.0;
	...
	continue;
      ...
      if(steepest_edge)
      {
	for(map<int, int>::iterator ite2 = varInd2ConstMap[ite->first].begin(); ite2 != varInd2ConstMap[ite->first].end(); ++ite2)
	{
	  int ib = ite2->first;
	  a2 += sq(pB[ib].CoeffMap[ite->first]);
	}
  ...
  //int Function::getXpivodRow_r2(int &xpivod, vector<Function> &B, map<int, map<int, int> > &varInd2ConstMap, bool &Flag)
  int Function::getXpivodRow(int &xpivod, vector<Function> &B, map<int, map<int, int> > &varInd2ConstMap)
  {
    int xpivodrow = 10000000;
    int xpivodrow2 = 10000000;
    ...
    {
      b = B[i].constTermVal;
      if(b < -1.0e-6) artificialFlag = true;
      if(artificialFlag) break;
    }

    dxpivodrow = artificialFlag ? -10000000 : 10000000;

    for(map<int, int>::iterator ite = varInd2ConstMap[xpivod].begin(); ite != varInd2ConstMap[xpivod].end(); ++ite)
    //for(int i = 0; i < Nconstraint; ++i)
    {
      int i = ite->first;
      Function *gfunc = &B[i];
  ...
  int Function::getXpivodRow(int &xpivod, vector<Function> &B)
  {
	...
	if(dx >= 0 && dxpivodrow >= dx)
	{
	  if(dxpivodrow == dx && xpivodrow2 <= gfunc->ix)
	   continue; 
  ...
  void Simplex::getFeasibleInitilize(int *basicVariableIdx, int *nonbasicVariableIdx)
  {
	...
	pB->at(ic).SwapVariable(xpivod, ix ,pB->at(xpivodrow));
      ...
      cout << fobj->constTermVal << " " << xpivod << endl;
    }
