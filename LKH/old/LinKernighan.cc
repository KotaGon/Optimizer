...
namespace myOptimizeName
{
  void LKSolver::RunLoop()
  {
	...
	*WorstCost = Cost;
      ...
      //Now = static_cast<double>((clock() - Last_time) / CLOCKS_PER_SEC); //difftime(clock(), Last_time) / CLOCKS_PER_SEC;
      Elapsed = getElapsed(getNow(), Start);
      Now = (double) clock()/CLOCKS_PER_SEC - Last_time;
      cout << "Run[" << Run << "], Cost = " << Cost << ", BestCost = " << *BestCost << ", Seed = " << Seed << ", Time = " << Now << endl;
  ...
  double LKSolver::FindTour()
  {
	...
	AdjustCandidateSets();
	Hash.clear();
	Hash[(int) Cost*Precision] = Hash.size();
	//Elapsed = getElapsed(getNow(), Start);
	...
	for(int X2 = 1; X2 <= 2; ++X2)
	{
	  ...
	  while((t2 = BestKOptMove(t1, t2, &G0, &Gain)))
	  {
	    if(Gain > 0)
	    {
	      ...
	      do
	      {
	        cost2 += Data->C(k, SUC(k)) - k->Pi - SUC(k)->Pi; 
	      }
	      while((k = SUC(k)) != Data->FirstNode);
	      cout << Cost - Gain << " " << cost2 << endl;
	       */
	      //cout << "get improve route " << endl;
	      //out_tour("improve" + to_string(out) +".gnu");
	    //  cout << Cost - Gain << endl;
		...
		t1->V = 1;
	      ...
	      if(Hash.find((int) Cost) != Hash.end())
