...
//      ((Simplex *)simplex)->optRevised();
  ...
  void BranchAndCut::outputRoute(OptClass *Opt)
  {
    ...
    for(int i = 0; i < Nconstraint; ++i)
    {
      ...
      double x_f = nodes[fromIdx].getPosx();
      double x_t = nodes[toIdx].getPosx();
      double y_f = nodes[fromIdx].getPosy();
      double y_t = nodes[toIdx].getPosy();

      tot += sqrt(sq(x_f - x_t) + sq(y_f - y_t));
